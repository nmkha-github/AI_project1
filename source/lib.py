...
def visualize_maze(matrix, bonus, start, end, route=None, visited=None):
    ...
    walls=[(i,j) for i in range(len(matrix)) for j in range(len(matrix[0])) if matrix[i][j].cost==math.inf]
    visited=[(i,j) for i in range(len(matrix)) for j in range(len(matrix[0])) if matrix[i][j].previous]
